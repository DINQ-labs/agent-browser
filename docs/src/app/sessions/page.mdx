...
Bridge callers can pass the same profile path in `POST /sessions` with `"profile": "/path/to/profile"`.
...
Bridge callers can set the same persistence handle with `"sessionName":"twitter"` in `POST /sessions`.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>AGENT_BROWSER_PROFILE</code></td><td>Persistent browser profile directory path</td></tr>
    <tr><td><code>AGENT_BROWSER_STATE</code></td><td>Storage state JSON file path</td></tr>
  </tbody>
</table>

## Runtime-specific note

If you launch sessions with `AGENT_BROWSER_RUNTIME=camoufox` or bridge `"runtime":"camoufox"`, the same profile and session persistence rules still apply. Camoufox uses Firefox internally and does not support Chromium-only features such as extensions and `--allow-file-access`.
