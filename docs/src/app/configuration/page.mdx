...
Patchright remains the default runtime. Runtime selection is currently **not** a config-file key or CLI flag. Use one of these entry points instead:

<table>
  <thead>
    <tr><th>Path</th><th>How to select Camoufox</th></tr>
  </thead>
  <tbody>
    <tr><td>Daemon / bridge env</td><td>Set <code>AGENT_BROWSER_RUNTIME=camoufox</code>.</td></tr>
    <tr><td>HTTP bridge</td><td>Send <code>"runtime":"camoufox"</code> in <code>POST /sessions</code>.</td></tr>
  </tbody>
</table>

Camoufox uses Firefox internally and rejects Chromium-only features such as extensions and <code>--allow-file-access</code>. If the runtime is not installed yet, run <code>npx camoufox-js fetch</code>.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>AGENT_BROWSER_RUNTIME</code></td><td>Browser runtime for daemon launches: <code>patchright</code> or <code>camoufox</code>.</td><td><code>patchright</code></td></tr>
    <tr><td><code>AGENT_BROWSER_PROFILE</code></td><td>Persistent browser profile directory path.</td><td>(none)</td></tr>
    <tr><td><code>AGENT_BROWSER_STATE</code></td><td>Storage state JSON file path.</td><td>(none)</td></tr>
    <tr><td><code>AGENT_BROWSER_STEALTH_WEBGL</code></td><td>Spoof WebGL vendor/renderer when SwiftShader is detected. Set to <code>0</code> to disable.</td><td>(enabled)</td></tr>
    ...
    <tr><td><code>AGENT_BROWSER_USER_AGENT</code></td><td>Default user agent override for daemon launches.</td><td>(none)</td></tr>
